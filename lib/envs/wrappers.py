...
Description: Wrappers for all environments. Some are adapted from Danijar Hafner's implementation
"""
...
from . import base
from ..common import space as spacelib
...
class NormalizeAction(base.Wrapper):
  ...
  def step(self, action):
    ...
    orig = self.act_space[self._key].unnormalize(action[self._key]).astype(np.float32)
...
class EnvStepLambdaWrapper(base.Wrapper):
  def __init__(self, env: base.Env, env_step_lambda: EnvStepLambda):
    ...
    self._internal_env = env

  @functools.cached_property
  def obs_space(self):
    spaces = self._internal_env.obs_space
  ...
  @functools.cached_property
  def act_space(self):
    return {**self._internal_env.act_space}

  def step(self, action):
    obs = self._internal_env.step(action)
    additional_obs = self._env_step_lambda({**obs, **action})
    return {**obs, **additional_obs}

class MeasureSuccessRate(EnvStepLambda):
  def __init__(self, env: base.Env, **kwargs) -> None:
    self.env = env
    self.success_fn = self._get_success_fn()
  ...
  def _get_success_fn(self) -> Callable[[ObservationAndPreviousActionType], bool]:
    default_success_fn = lambda o: False
    try:
      success_fn = getattr(self.env, "is_success")
    except:
      print(f"[MeasureSuccessRate] You have not implemented the success function for this enivonment, falling back to default. Is this a mistake?", color='yellow')
...
class UnifyDtypes(base.Wrapper):
