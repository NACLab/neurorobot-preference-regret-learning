...
Description: Mountain Car Continuous Gym environment
"""
...
from ..from_env.gym import GymEnv
...
class MountainCarContinuous(gym.Env):
  ...
  * v0: Initial versions release (1.0.0)
  ...
  def __init__(self, render_mode: Optional[str] = None, goal_velocity=0,
        easy_mode=True, reward_shaping=False, freeze_when_done=False, max_time_steps=200):
  ...
  def step(self, action: np.ndarray):
      ...
      self.time_step += 1
      return self.state, reward, False if self.freeze_when_done else self.terminated, self.time_step >= self.max_time_steps, {}
...
class MountainCarGeneralEnv(GymEnv):
  def __init__(self, task, obs_key='image', act_key='action', **kwargs):
    if 'render_mode' not in kwargs:
      kwargs['render_mode'] = 'rgb_array'
    super().__init__(env=gym.make("mtc", **kwargs), obs_key=obs_key, act_key=act_key)

  def is_success(self, obs):
    position = obs['state'][0]
    velocity = obs['state'][1]
    if self._env.unwrapped.easy_mode:
      return bool(position >= self._env.unwrapped.goal_position)
    else:
      return bool(position >= self._env.unwrapped.goal_position and velocity >= self._env.unwrapped.goal_velocity)

class MountainCarExpertDataset(ExpertDataset):
  def __init__(self, task, text_processor: TextProcessor | None = None, seed: int | None = None, **kwargs):
    # print(f"[build_expert_dataset] [mtc] Building expert dataset for task: {task}: {kwargs}")
    self._env = MountainCarGeneralEnv("mtc", **kwargs)
  ...
  def dataset(self, final_env: Env):
    while True:
      obs = final_env.step({'action': self._normalize_action(final_env.act_space['action'].sample()), 'reset': True})
      while True:
        # Heuristic: push left if on the left, right if on the right
        if obs['state'][0] < 0:
          if obs['state'][1] < 0:
            action = -1.0
          else:
          ...
          if obs['state'][1] < 0:
            ...
            action = 1.0
        action = self._normalize_action(np.array([action], dtype=np.float32))
        transition = {**obs, 'action': action}
        yield transition
        obs = final_env.step({'action': action, 'reset': False})
  ...
  def _normalize_action(self, action):
    return self._env.act_space['action'].normalize(action)