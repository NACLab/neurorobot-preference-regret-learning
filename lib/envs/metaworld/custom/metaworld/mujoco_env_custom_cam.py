...
class MujocoEnv(BaseMujocoEnv):
    ...
    def __init__(
        ...
        width: int = DEFAULT_SIZE,
        height: int = DEFAULT_SIZE,
        obs_width: int = DEFAULT_SIZE,
        obs_height: int = DEFAULT_SIZE,
        camera_id: Optional[int] = None,
        camera_name: Optional[str] = None,
        ...
        if isinstance(default_camera_config, List):
            ...
            self.mujoco_renderers = [
                MujocoRenderer(
                    self.model, self.data, camera_config, width, height,
            ...
            self.mujoco_obs_renderers = [
                MujocoRenderer(
                    self.model, self.data, camera_config, obs_width, obs_height,
                    camera_id=camera_id, camera_name=camera_name
                ) for camera_config in default_camera_config
            ]
        elif isinstance(default_camera_config, dict) or default_camera_config is None:
            self.mujoco_renderer = MujocoRenderer(
                self.model, self.data, default_camera_config, width, height,
                camera_id=camera_id, camera_name=camera_name
            )
            self.mujoco_renderers = [self.mujoco_renderer]
            self.mujoco_obs_renderer = MujocoRenderer(
                self.model, self.data, default_camera_config, obs_width, obs_height,
                camera_id=camera_id, camera_name=camera_name
            )
        else:
    ...
    def render(self):
        return [renderer.render(self.render_mode) for renderer in self.mujoco_renderers]

    def render_obs(self):
        return [renderer.render(self.render_mode) for renderer in self.mujoco_obs_renderers]
