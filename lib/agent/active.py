...
from .base import BaseActiveAgent
from ..nn import internal
from ..nn import transform
from ..nn import ninjax as nj
from ..common import Path, print, timer, Counter, Prefetch
...
class JAXAgent(BaseActiveAgent):

  def __new__(subcls, obs_space, act_space, config, *args, **kwargs):
    ...
    model.__init__(obs_space, act_space, config, *args, **kwargs)
  ...
  def train(self, carry, data):
    ...
    if self.jaxcfg.profiler:
      ...
      if self.n_updates == 120:
        ...
        if copyto:
          for subdir in Path(outdir).glob('*'):
