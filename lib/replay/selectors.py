...
class EpochBased:
  """Epoch-based selector that goes through all data once per epoch."""

  def __init__(self, seed=0):
    self.indices = {} # This is for O(1) deletions
    self.keys = []
    self.rng = np.random.default_rng(seed)
    self.epoch_keys = []
    self.epoch_index = 0
  ...
  def __call__(self):
    if not self.keys:
      raise IndexError("No items to select from")

    # Start new epoch if needed
    if self.epoch_index >= len(self.epoch_keys):
      self.epoch_keys = self.keys.copy()
      self.rng.shuffle(self.epoch_keys)
      self.epoch_index = 0

    # Get next item in epoch
    key = self.epoch_keys[self.epoch_index]
    self.epoch_index += 1
    return key

  def __setitem__(self, key, _):
    # stepids parameter ignored for classification
    self.indices[key] = len(self.keys)
    self.keys.append(key)
    # Reset epoch when new data is added
    self.epoch_keys = []
    self.epoch_index = 0

  def __delitem__(self, key):
    # O(1) deletion using swap-with-last pattern
    index = self.indices.pop(key)
    last = self.keys.pop()
    if index != len(self.keys):
      self.keys[index] = last
      self.indices[last] = index
    # Reset epoch when data is removed
    self.epoch_keys = []
    self.epoch_index = 0

  def reset_epoch(self):
    """Manually reset to start a new epoch."""
    self.epoch_keys = []
    self.epoch_index = 0
