...
class Agg:
  ...
  def _add_single(self, key, value, agg, prefix):
    ...
    if reducers:
      ...
      return
    if agg == 'default':
      agg = 'avg' if np.asarray(value).ndim <= 1 else 'last' # this one is the one that make image automatically last, and scalar and array automatically mean
...
class Concat:
  ...
  def __init__(self, initial, maxlen=1e6):
    self.array = [initial]
    self.maxlen = int(maxlen)
  ...
  def current(self):
    return np.concatenate(self.array)
